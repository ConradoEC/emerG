...
.then((response) => {
    ...
    response.nichos.forEach(item => {
        inputNiche.innerHTML += `<option value="${item.nicho}" class="options">${item.nicho}</option>` 
    })

    response.cidades.forEach(item => {
        inputRegion.innerHTML += `<option value="${item.city}" class="options">${item.city}</option>` 
    })
})
.catch((error) => {
    console.log(error)
...
function tagOngs(value)
{
    if(filterPlaces.length == 0)
    {
        const div_ong = document.createElement('div')
            ...
            <a href="../Perfil/?jjp=${value._id}" class="ong_perfil_link" id="${value.ong_name}${value._id}">Perfil emerG</button>
            `
    ...
    else
    {
        generalOngs.forEach(item => 
        {
            if(item._id == value)
            {
                ...
                <a href="../Perfil/?jjp=${item._id}" class="ong_perfil_link" id="${item.ong_name}${item._id}">Acessar perfil</button>
                `
...
function inputs()
{
    if(window.getComputedStyle(formsAddMarker_CreateMiniMap,'::before').display == 'block')
    {
        if(formsAddMarker_input[0].value != '' && formsAddMarker_input[1].value != '' && miniMap_floodLevel != 0)
        {
            ...
            newMarkerData.userName = sessionStorage.getItem('nome')
            newMarkerData.userEmail = sessionStorage.getItem('email')
            newMarkerData.userPhone = sessionStorage.getItem('telefone')
...
function filtrar()
{
    ...
    searchBar.style.setProperty('background-color', 'transparent')
    ...
    ul.addEventListener('click', (event) =>
    {
        ...
        filter_icon.innerText = 'filter_alt'
    })
}
...
{
    if(filter_icon.innerText == 'close')
    {
        document.getElementById('choose_searchBarFilter_contentOptions').style.setProperty('display', 'none')
        // choose_searchBarFilter_content.style.setProperty('background-color', '#a0d3ff')
        searchBar.style.setProperty('background-color', '#a0d3ff')
        searchBar.value = ''
        filter_icon.innerText = 'filter_alt'
    }
...
formsButtonReset.addEventListener("click", () => {
    inputNiche.value = 0
    inputRegion.value = 0
    stars = 0
    for(i = 0; i < 5; i++)
...
formsAddMarker_btnClose.addEventListener('click', function()
{
    ...
    marker = new google.maps.Marker({
        position: {lat: -23.527471502557056, lng: -46.85516892549204},
        map: '',
        title: ``,
        icon: ''
    })
...
btnAddNewMarker.addEventListener('click', function()
{
    ...
    if(btnAddNewMarker.style.backgroundColor == 'rgb(13, 116, 229)' && newMarkerData.lat != 0)
    {
        ...
        .then((response) => 
        {
            ...
            $.ajax({
                url: 'https://api-emer-g.vercel.app/markersFlood',
                method: 'GET'
            })
            .then((response) => 
            {
                generalMarkers = response
                initMap()
            })
            .catch((error) => 
            {
...
formsAddMarker_ReloadMiniMap.addEventListener('click', async function()
{
    ...
    switch (true)
    {
        case formsAddMarker_input[2].value == 1: 
            formsAddMarker_labelFloodLevel.style.setProperty('background-color', 'rgba(242,225,33,255)')
            formsAddMarker_labelFloodLevel.innerText = 'Baixo'
            miniMap_floodLevel = await 1    
            break
        case formsAddMarker_input[2].value == 2: 
            formsAddMarker_labelFloodLevel.style.setProperty('background-color', 'rgba(253,166,33,255)')        
            formsAddMarker_labelFloodLevel.innerText = 'Moderado'
            miniMap_floodLevel = await 2
            break
        case formsAddMarker_input[2].value == 3: 
            formsAddMarker_labelFloodLevel.style.setProperty('background-color', 'rgba(194,39,45,255)')    
...
formsAddMarker_input[2].addEventListener('click', function()
{
    switch (true)
    {
        case formsAddMarker_input[2].value == 1: 
...
async function initMap(value)
{
    if(isMapWater == false)
    {
                ...
                {                
                    ...
                    <div class="markerContent">
                        ...
                        <div class="markerContent_info">
                            ...
                            <p class="info_phone">${mapPlacesSetting[`newName${filterPlaces_item}`].phone}</p>
                        </div>
                    </div>
                    `

                    infoWindow = new google.maps.InfoWindow({
                        content: `<h1 class="sla2">` + `${mapPlacesSetting[`newName${filterPlaces_item}`].item}` + `</h1>`,
                        ariaLabel: "sans-serif"
                    })
        ...
        else
        {
            ...
            switch (miniMap_floodLevel)
            {
                case 1: 
                    flood_level = 'Baixa'
                    colorIcon = 'https://i.ibb.co/b32Yhmt/Ponto-amarelo.png'
                    break
                case 2: 
                    flood_level = 'Moderada'
                    colorIcon = 'https://i.ibb.co/TBKGQH8/Ponto-laranja.png'
                    break
                case 3: 
                    flood_level = 'Alta'
                    colorIcon = 'https://i.ibb.co/NgqZ252/Ponto-vermelho.png'
                    break
                default: 
                    flood_level = ''
                    colorIcon = ``
    ...
    else
    {
        ...
        generalMarkers.forEach((item) => 
        {
            ...
            const waterMarkersContent = `
            <div class="waterMarkerContent">
                <h1 class="waterMarkerFlood">Nome do usuário: ${item.userName}</h2>
                <br>
                <h3 class="waterMarkerFlood">E-mail do usuário: ${item.userEmail}</h2>
                <br>
                <br>
                <h2 class="waterMarkerFlood">Possibilidade de enchente: ${flood_level}</h2>
                <br>
                <h3 class="waterMarkerAddress">Endereço: ${item.address}</h3>
            </div>
            `

            infoWindow = new google.maps.InfoWindow({
                content: waterMarkersContent,
                ariaLabel: 'slameu',
